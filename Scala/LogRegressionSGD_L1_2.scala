 ...
 */
object LogRegressionSGD_L1_2 {
  def main(args: Array[String]) {

    val sc = new SparkContext("local[2]", "LogRegressionSGDL2")
    ...
    model.optimizer.setRegParam(0.01).setNumIterations(50).setStepSize(5)
    ...
    val lrTotalCorrect = training.map { point =>
      if (lrModelSGD.predict(point.features) == point.label) 1 else 0
    }.sum
    ...
    lrModelSGD.clearThreshold()

    val scoreAndLabels = training.map { point =>
      val score = lrModelSGD.predict(point.features)
      (score, point.label)
    }
    ...
    val scoreAndLabels1 = test.map { point =>
      val score1 = lrModelSGD.predict(point.features)
      (score1, point.label)
    }
